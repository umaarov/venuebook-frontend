...
import {useAdminCreateWeddingHallMutation, useAdminUpdateWeddingHallMutation} from '../../features/admin/adminApi';
...
const OwnerManageWeddingHallPage = ({mode}) => {
    ...
    } = useGetWeddingHallByIdQuery(hallIdParam, {
    ...
    return (
        <div className="container">
            ...
            <form onSubmit={handleSubmit}>
                <div><label htmlFor="wh-name">Name:</label><input type="text" id="wh-name" value={name}
                                                                  onChange={(e) => setName(e.target.value)} required/>
                </div>
                <div><label htmlFor="wh-address">Address:</label><input type="text" id="wh-address" value={address}
                                                                        onChange={(e) => setAddress(e.target.value)}
                                                                        required/></div>
                <div><label htmlFor="wh-capacity">Capacity (guests):</label><input type="number" id="wh-capacity"
                                                                                   value={capacity}
                                                                                   onChange={(e) => setCapacity(e.target.value)}
                                                                                   required min="1"/></div>
                <div><label htmlFor="wh-pricePerSeat">Price Per Seat ($):</label><input type="number" step="0.01"
                                                                                        id="wh-pricePerSeat"
                                                                                        value={pricePerSeat}
                                                                                        onChange={(e) => setPricePerSeat(e.target.value)}
                                                                                        required min="0"/></div>
                <div><label htmlFor="wh-phone">Phone:</label><input type="tel" id="wh-phone" value={phone}
                                                                    onChange={(e) => setPhone(e.target.value)}
                                                                    required/></div>
                <div>
                    <label htmlFor="wh-districtId">District:</label>
                    <select id="wh-districtId" value={districtId} onChange={(e) => setDistrictId(e.target.value)}
                            required>
                ...
                */}
                ...
                {currentImageFiles.length > 0 && (
                    <div>
                        ...
                        <select id="wh-primaryImageIndex" value={primaryImageIndex === null ? '' : primaryImageIndex}
                                onChange={(e) => setPrimaryImageIndex(e.target.value === '' ? null : parseInt(e.target.value, 10))}>
                ...
                <button type="submit"
                        disabled={isLoadingSubmit}>{isLoadingSubmit ? 'Saving...' : (mode === 'create' ? 'Create Hall' : 'Update Hall')}</button>
            </form>
        </div>
    );
};
